...
void aes_encode(struct string_st *res, const struct string_st *msg, const struct string_st *key) {
    if (res == NULL) return;
    if (msg == NULL || key == NULL) return string_clear(res);

    aes_base *aes = aes_init();
    aes_ctx _msg;
    size_t size = msg->size;
    char _size = (char)(msg->size % 16);
    size_t res_size = 0;

    unsigned times = msg->size / 64 + (int) (msg->size % 64 != 0);
    string_resize(res, times * 64 + 1);

    for (unsigned i = 0; i < times; i++) {
        _msg.length = 64;
        if (_msg.length > size) _msg.length = size;
        size -= _msg.length;
        _msg.ctx = (unsigned char *) (msg->data + i * 64);

        aes_ctx out = encrypt(_msg, (unsigned char *) key->data, aes);
...
void aes_decode(struct string_st *res, const struct string_st *msg, const struct string_st *key) {
    ...
    size_t _size = (unsigned char)msg->data[size];
    size_t res_size = 0;

    unsigned times = size / 64 + (int) (size % 64 != 0);
    string_resize(res, times * 64);
    ...
    for (unsigned i = 0; i < times; i++) {
        ...
        aes_ctx out = decrypt(_msg, (unsigned char *) key->data, aes);
...
struct crypto_code aes_code = {&aes_encode, &aes_decode};